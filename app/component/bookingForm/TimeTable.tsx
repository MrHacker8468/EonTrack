...
async function getData(userName: string, selectedDate: Date) {
    ...
    const nylasCalendarData = await nylus.calendars.getFreeBusy({
...
//@ts-ignore
    const busySlots = nylasData.data[0].timeSlots.map((slot: { startTime: number; endTime: number; }) => (
        {
    ...
    const freeSlots = allSlots.filter(slot => {
        ...
        return(
            isAfter(slot, now) && !busySlots.some(
